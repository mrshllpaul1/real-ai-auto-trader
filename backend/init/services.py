...
async def _init_phase3_ai(db):
    ...
    # Hidden Gem Predictor (rainbow_agent is deferred, pass None)
    hidden_gem_predictor = get_gem_predictor(db, market, None)
...
async def _init_phase6_scheduling(db):
    ...
    weekly_scheduler = await initialize_weekly_scheduler(db, _services['coin_selector'], _services.get('auto_trader'))
...
async def _init_phase7_wire_dependencies(db):
    ...
    # Enhanced MTF Training Routes (with sentiment integration)
    from routes import enhanced_mtf_training as enhanced_mtf_training_routes
    enhanced_mtf_training_routes.set_dependencies(db, _services['enhanced_mtf_training'])
