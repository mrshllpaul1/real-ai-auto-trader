...
class TestRLAgentP0:
    ...
    def test_rl_agent_status_trained(self):
        ...
        response = requests.get(f"{BASE_URL}/api/predictions/rl-agent/status", timeout=10)
        assert response.status_code == 200
        data = response.json()
...
class TestAutoSpotScanner:
    ...
    def test_auto_spot_scan_status(self):
        ...
        response = requests.get(f"{BASE_URL}/api/training-scheduler/auto-spot-scan/status", timeout=10)
        assert response.status_code == 200
        data = response.json()
...
class TestSpotTrading:
    ...
    def test_spot_trading_status(self):
        ...
        response = requests.get(f"{BASE_URL}/api/spot/status", timeout=10)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_spot_positions(self):
        ...
        response = requests.get(f"{BASE_URL}/api/spot/positions", timeout=10)
        assert response.status_code == 200
        data = response.json()
...
class TestTrainingDashboard:
    ...
    def test_training_history(self):
        ...
        response = requests.get(f"{BASE_URL}/api/training-history", timeout=10)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_training_scheduler_status(self):
        ...
        response = requests.get(f"{BASE_URL}/api/training-scheduler/status", timeout=10)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_learning_status(self):
        ...
        response = requests.get(f"{BASE_URL}/api/learning/status", timeout=10)
        assert response.status_code == 200
        data = response.json()
...
class TestServerModularization:
    ...
    def test_kraken_balance(self):
        ...
        response = requests.get(f"{BASE_URL}/api/kraken/balance", timeout=15)
    ...
    def test_isolated_portfolio(self):
        ...
        response = requests.get(f"{BASE_URL}/api/isolated-portfolio/status", timeout=10)
        assert response.status_code == 200
        data = response.json()
