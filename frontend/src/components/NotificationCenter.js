...
import { Bell, X, Check, CheckCheck, Volume2, Phone } from 'lucide-react';
  ...
  return (
    <div className="relative" ref={dropdownRef} data-testid="notification-center">
      ...
      >
        ...
        {unreadCount > 0 && (
          <Badge 
            className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center p-0 bg-[#FF0055] text-white text-xs"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 mt-2 w-80 bg-[#0A0A0A] border border-[#1F1F1F] rounded-lg shadow-xl z-50 overflow-hidden"
            ...
            <div className="flex items-center justify-between p-3 border-b border-[#1F1F1F]">
              ...
              {unreadCount > 0 && (
                <Button
                  ...
                  size="sm"
                  onClick={markAllRead}
                  className="text-xs text-[#00FF94] hover:text-[#00FF94]"
            ...
            <div className="max-h-96 overflow-y-auto">
              {notifications.length > 0 ? (
                notifications.map((notif) => (
                  <div
                    key={notif.id}
                    className={`p-3 border-b border-[#1F1F1F] hover:bg-[#121212] cursor-pointer transition-colors ${
                      !notif.read ? 'bg-[#00FF94]/5' : ''
                    }`}
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      {!notif.read && (
                        <div className="w-2 h-2 rounded-full bg-[#00FF94] mt-2" />
                      )}
                    </div>
                  </div>
                ))
              ) : (
                <div className="p-6 text-center" data-testid="no-notifications">
