...
const AdvancedFeatures = () => {
  ...
  const getTradesNeeded = () => {
    if (!rebalanceData) return [];
    return rebalanceData.trades_needed || [];
  };

  const getTotalValue = () => {
    if (!rebalanceData) return 0;
    return rebalanceData.total_value || 0;
  };
    ...
    <div className="p-6 lg:p-12 space-y-6" data-testid="advanced-features">
      ...
      <Tabs defaultValue="backtest" className="space-y-6">
        <TabsList className="bg-[#0A0A0A] border border-[#1F1F1F]">
          <TabsTrigger value="backtest" data-testid="backtest-tab">
            <FlaskConical size={16} className="mr-2" />Backtesting
          </TabsTrigger>
          <TabsTrigger value="rebalance" data-testid="rebalance-tab">
            <PieChart size={16} className="mr-2" />Rebalancing
          </TabsTrigger>
          <TabsTrigger value="social" data-testid="social-tab">
            <Users size={16} className="mr-2" />Social
          </TabsTrigger>
        </TabsList>
                ...
                {backtestResult ? (
                  <div className="grid grid-cols-2 gap-4" data-testid="backtest-results">
                    <div className="p-3 bg-[#121212] rounded-lg">
                      ...
                      <div className={`text-2xl font-data font-bold ${(backtestResult.total_return_pct || 0) >= 0 ? 'text-[#00FF94]' : 'text-[#FF0055]'}`}>
                        {(backtestResult.total_return_pct || 0) >= 0 ? '+' : ''}{formatReturn(backtestResult.total_return_pct)}%
                      </div>
                    </div>
                    <div className="p-3 bg-[#121212] rounded-lg">
                    ...
                    <div className="p-3 bg-[#121212] rounded-lg">
                      ...
                      <div className="text-2xl font-data font-bold text-[#00FF94]">
                        ${(backtestResult.final_value || 0).toLocaleString()}
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-8" data-testid="backtest-empty">
        ...
        <TabsContent value="rebalance" className="space-y-6">
          <Card className="bg-[#0A0A0A] border-[#1F1F1F]">
            ...
            <CardContent>
              {getTradesNeeded().length > 0 ? (
                <div className="space-y-3" data-testid="rebalance-trades">
                  <div className="text-lg font-bold mb-4">
                    Total Value: ${getTotalValue().toLocaleString()}
                  </div>
                  {getTradesNeeded().map((trade, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-[#121212] rounded-lg">
                      ...
                      <Badge className={trade.action === 'BUY' ? 'bg-[#00FF94]/20 text-[#00FF94]' : 'bg-[#FF0055]/20 text-[#FF0055]'}>
                        {trade.action} ${(trade.value_usd || 0).toFixed(0)}
                      </Badge>
                    </div>
                  ))}
                  <Button
                    onClick={executeRebalance}
                    className="w-full bg-[#00FF94] hover:bg-[#00CC76] text-black font-bold rounded-full mt-4"
                    data-testid="execute-rebalance-btn"
                  >
                    <RefreshCw size={16} className="mr-2" />Execute Rebalance
                  </Button>
                </div>
              ) : (
                <div className="text-center py-8" data-testid="rebalance-empty">
              ...
              {leaderboard.length > 0 ? (
                <div className="space-y-2" data-testid="leaderboard">
                  {leaderboard.map((trader, i) => {
                    const profitPct = trader.total_profit_pct || 0;
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between p-3 bg-[#121212] rounded-lg">
                        ...
                        <div className="text-right">
                          <div className={`font-data ${profitPct >= 0 ? 'text-[#00FF94]' : 'text-[#FF0055]'}`}>
                            {profitPct >= 0 ? '+' : ''}{profitPct.toFixed(1)}%
                          </div>
                          <div className="text-xs text-[#A1A1AA]">{winRate.toFixed(0)}% win</div>
                        </div>
                      </div>
                    );
                  })}
